...
export class ComponentRegistry {
    ...
    getRef(componentConstructor: ComponentConstructor) {
        ...
        if (!componentInstance) {
            const dependencies = this.dependenciesMapping.get(componentConstructor) ?? [];
            const serviceInstances = dependencies.map((serviceConstructor) => serviceRegistryInstance.getRef(serviceConstructor));
            console.log(`[ComponentRegistry] services to be injected to [${componentConstructor.name}]:`, serviceInstances.map((service) => service.constructor.name));
