...
export class ServiceRegistry {
  private static instance: ServiceRegistry | null;
  ...
  registerDependencies(
    ...
    if (this.dependenciesMapping.has(serviceConstructor)) {
      throw new Error(
        `Service Dependencies already exists for ${serviceConstructor.name}`,
      );
    }
    this.dependenciesMapping.set(serviceConstructor, dependencies);
    console.log(
      `[ServiceRegistry] [${serviceConstructor.name}]'s dependencies registered:`,
      dependencies,
    );
  }
